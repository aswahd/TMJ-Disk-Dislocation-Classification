...
TMJ_folder = 'E:/AI_Summer/TMJDATA'
...
def find_yellow_ratio(c_disc, emcon_line_points, pn, sn):
    ...
    plt.show()
    plt.draw()
    fig1.savefig(f"E:/AI_Summer/mask_ratio_plots/{pn}_{sn}", dpi=100)
...
sorted_patient_folder = []
i = 0

for patient_folder in os.listdir(TMJ_folder):
    sorted_patient_folder.append(int(patient_folder))

sorted_patient_folder.sort()

for patient_folder in sorted_patient_folder: # Go through each patient's folder
    patient_number = patient_folder # IMPORTANT TO KEEP TRACK OF PATIENT'S NUMBER
    i += 1
    # print(patient_folder)
    for label_folder in sorted(os.listdir(f"{TMJ_folder}/{patient_folder}")): # Go inside the Label folder
        if not 'Label' in label_folder:
            continue
        
        else:
          
            for file in os.listdir(f"{TMJ_folder}/{patient_folder}/{label_folder}"):
                if not 'Slice_' in file:
                    continue
                fullFileName = f"{TMJ_folder}/{patient_folder}/{label_folder}/{file}"
                fullFileName_npz = re.sub('json', 'npz', fullFileName)
                # Opening JSON file
                if not 'json' in fullFileName:
                    continue
                ...
                slice_number = int(re.sub('_', '', slice_number))
                
                # Create image based on json data
                if 'Data' in data.keys():
                    # print(data['Data'].keys())
                    if all(elm in data['Data'].keys() for elm in ['Disc','Condyle','Eminence'] ):
                        disc = data['Data']['Disc']
                        condyle = data['Data']['Condyle']
                        eminence= data['Data']['Eminence']
                        
                        if all([disc,condyle,eminence]):
                            try:
                                slices.append(int(file.split('_')[-1].split('.json')[0]))
                            except:
                                continue
                            #print("Read contour points")
                            ## We could use Points instead
                            c_disc = [tuple(pt) for pt in disc[0]['Contour']]
                            c_condyle  = [tuple(pt) for pt in condyle[0]['Contour']]
                            c_eminence = [tuple(pt) for pt in eminence[0]['Contour']]
                            ...
                            plt.imshow(mask)
                            plt.show()    
                            image_name = f"E:/AI_Summer/mask_images_updated/{re.sub('json', 'png', file)}"
                            image_name = re.sub('Slice', f'{patient_number}', image_name)
...
excel_path = "E:/AI_Summer/TMJ_Updated_Calc.xlsx"
